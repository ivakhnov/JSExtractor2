...
///////////////////////////////////////////////////////////////////////////////////////
    ...
    getScripts : function(pageUrl, callback) {
        this.prototype._client(function(client) {
            // First get the amount of stored scripts
            client.hget('page:'+pageUrl, 'scripts:count', function(err, count) {
                var results = [];
                // If the return value is 'null' it means no scripts are present in database
                if(count == null) { 
                    return callback(results);
                // Otherwise get all the scripts (loop through all id's)
                } else {
                    for(var i = 0; i < count; i++) {
                        client.hget('page:'+pageUrl, 'scripts:'+(i+1), function(err, script) {
                            results.push(JSON.parse(script));
    ...
    getEvents : function(pageUrl, callback) {
        this.prototype._client(function(client) {
            // First get the amount of stored events
            client.hget('page:'+pageUrl, 'events:count', function(err, count) {
                var results = [];
                // Return value is 'null' means nothing is stored in database yet
                if(count == null) {
                    return callback(results);
                // Otherwise get all the id's
                } else {
                    for(var i = 0; i < count; i++) {
                        client.hget('page:'+pageUrl, 'events:'+(i+1), function(err, event) {
