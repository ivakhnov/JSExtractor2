...
///////////////////////////////////////////////////////////////////////////////////////
    ...
    this.savePage = function(pageUrl, scripts, events, frameworks, callback) {
    	_client.sadd('pagesList', pageUrl, function(err, addedCount) {
    		if(err) { callback(err); } 
    		else if(addedCount == 0) { callback(null); }
    		else { 
    			async.parallel([
				    function(callback){
				    	_saveScripts(pageUrl, scripts, function(err, returnValue) {
				    		if (err) { callback(err); }
				    		else { callback(null, returnValue); };
				    	});
				    },
				    function(callback){
				        _saveEvents(pageUrl, events, function(err, returnValue) {
				    		if (err) { callback(err); }
				    		else { callback(null, returnValue); };
				    	});
				    },
				    function(callback){
				        _saveFrameworks(pageUrl, frameworks, function(err, returnValue) {
				    		if (err) { callback(err); }
				    		else { callback(null, returnValue); };
				    	});
				    },
    		...
    		if(err) { callback(err); }
    			...
    			async.parallel({
				    ...
				    all : function(callback){
				    	_client.zrange('scripts:'+pageUrl, 0, -1, function(err, reply) {
				    		if(err) { callback(err); } 
				    ...
				    inplace : function(callback){
				    	_getScriptsByType(pageUrl, count, 'inplace', function(err, reply) {
				    		if(err) { callback(err); } 
							else { callback(null, reply); }
				    	});
				    },
				    ...
				    sourceFiles : function(callback){
				    	_getScriptsByType(pageUrl, count, 'file_nonCrossDomain', function(err, reply) {
				    		if(err) { callback(err); } 
							else { callback(null, reply); }
				    	});
				    },
				    ...
				    sourceFilesCrossDomain : function(callback){
				    	_getScriptsByType(pageUrl, count, 'file_crossDomain', function(err, reply) {
				    		if(err) { callback(err); } 
							else { callback(null, reply); }
				    	});
				    },
	...
	var _saveFrameworks = function(pageUrl, frameworks, callback) {
		var added_counter = 0;
    	if(frameworks.length == 0) { callback(null, 0); }
    	else {
	    	for(var i=0; i<frameworks.length; i++) {
	    		var framework = frameworks[i];
	    		_client.zadd('frameworks:'+pageUrl, i, framework, function(err) {
	    			added_counter++;
	    			_client.sadd('frameworksList', framework, function(err, reply) {
	    				if(err) { callback(err); }
	    			});
	    			_client.sadd('framework:'+framework, pageUrl, function(err, reply) {
	    				if(err) { callback(err); }
	    			});
	    			if(err) { callback(err); } 
	    			else if (added_counter == frameworks.length) { 
	    				callback(null, added_counter); 
	    			};
	    		});
	    	}
