...
///////////////////////////////////////////////////////////////////////////////////////
	...
	this.savePluginConfig = function(pluginName, confName, confDescription, confConfig, callback) {
		_getPluginID(pluginName, function(pluginID) {
			var err = null;
			_client.zadd('pluginConfigs:'+pluginID, 0, JSON.stringify({ 
						'confName' : confName, 
				...
				if (res == 0) {
					...
					_client.SET('pluginConfig:'+confName, 
							JSON.stringify(confConfig),
						function(res) {
							callback(err);
					});
				};
	 ...
	 * @return {JSON Object}            Object consistings out of a configuration name and description
	 */
	this.getPluginConfigs = function(pluginName, callback) {
		_client.HGET('plugins', pluginName, function(err, pluginID) {
			if (pluginID == null) { callback([]); }
				...
				_client.ZRANGE('pluginConfigs:'+pluginID, 0, -1, function(err, configs) {
					callback(configs);
				});
			};
		});
